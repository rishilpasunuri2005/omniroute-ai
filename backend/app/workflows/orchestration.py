...
class AgentWorkflow:
    ...
    async def run(self, request: ChatRequest, session: AsyncSession) -> ChatResponse:
        start = time.perf_counter()
        conversation_id = request.conversation_id or str(uuid.uuid4())
        state = await self.graph.ainvoke({"request": request, "conversation_id": conversation_id})
    ...
    async def _route_node(self, state: WorkflowState) -> dict[str, Any]:
        ...
        return {
            "route": route,
            "trace": [
                WorkflowStep(agent="Router Agent", status="completed", detail=f"Selected {route.selected_model}: {route.reason}"),
            ],
        }
    ...
    async def _planner_node(self, state: WorkflowState) -> dict[str, Any]:
        request = state["request"]
        route = state["route"]
        plan = await self.ollama.generate(route.selected_model, request.prompt, PLANNER_SYSTEM_PROMPT)
        return {
            "plan": plan,
            "trace": state["trace"] + [
    ...
    async def _execute_node(self, state: WorkflowState) -> dict[str, Any]:
        ...
        response = await self.ollama.generate(route.selected_model, composed_prompt, system_prompt)
        return {
            "response": response,
    ...
    async def _validate_node(self, state: WorkflowState) -> dict[str, Any]:
        ...
        if not validation.passed:
            ...
            response = await self.ollama.generate(route.fallback_model, retry_prompt, GENERAL_SYSTEM_PROMPT)
            validation = self.validator.validate(response, expected_json=self._expects_json(request.prompt))
            return {
                "response": response,
                "validation": validation,
